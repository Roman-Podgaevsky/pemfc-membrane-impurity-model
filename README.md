...
Расчетная схема собрана по открытым литературным источникам и реализована в Jupyter Notebook.
...
- перенос кислорода $O_2$ от катода к аноду;
- перенос азота $N_2$ от катода к аноду;
- перенос водорода $H_2$ от анода к катоду;
...
В репозитории показан пример реконструкции CAE-модели PEMFC в виде Python-расчета с пояснениями.

- сформулирована физическая постановка задачи;
- реализованы основные уравнения переноса компонентов через мембрану;
- использованы эмпирические корреляции для водосодержания и свойств мембраны;
- результаты оформлены в табличном виде для сопоставления с CAE-моделью;
- отдельно описаны принятые допущения, знаки потоков и ограничения расчета.
...
Для выполнения расчета требуется задать параметры батареи топливных элементов (БТЭ), состояния анода и катода, расчетный ток и параметры закрытой рециркуляции.
...
- молярные доли $H_2$, $N_2$, $H_2O$, $O_2$.
...
## Основные блоки jupyter notebook

### 1. Базовые параметры

Базовые параметры БТЭ, составы газовых смесей, давления, температуры, ток и физические константы задаются в первой ячейке.
...
Функции расчета параметров мембраны, влияющих на газообмен:
...
Разности парциальных давлений задают движущую силу для газового переноса.

Газовый перенос для $O_2$, $N_2$ и $H_2$ описан линейной зависимостью по перепаду парциальных давлений:
...
Коэффициент $n_{\mathrm{drag}}$ показывает количество воды, переносимой через мембрану на единицу протонного заряда. В PEMFC один моль прореагировавшего водорода соответствует двум молям электронов и двум молям переносимого протонного заряда.
...
Минус означает, что водород удаляется из анодного газового объема как реагирующий компонент. Число 2 в знаменателе связано с тем, что на один моль $H_2$ приходится два моля электронов.
...
Модель позволяет выполнять расчет квазистационарного накопления примесей в системе рециркуляции водорода при закрытых клапанах удаления примесей.

Учтены:

- относительная влажность на аноде;
- влагоотделитель;
- полное восполнение реакционного расхода $H_2$ чистым водородом;
- требуемый расход на входе в БТЭ через стехиометрическое число;
- накопление $O_2$, $N_2$ и $H_2O$ за заданный временной горизонт.
...
- `n_o2_to_anode_mol_s > 0` означает, что $O_2$ идет в анод;
- `n_n2_to_anode_mol_s > 0` означает, что $N_2$ идет в анод;
- `n_h2_to_cathode_mol_s > 0` означает, что $H_2$ идет в катод;
- `n_h2o_net_to_anode_mol_s > 0` означает, что суммарно вода идет в анод;
- отрицательное значение `n_h2_consumption_mol_s` означает реакционный расход водорода на аноде.
...
- скорость накопления $O_2$, $N_2$ и $H_2O$ в анодном контуре;
