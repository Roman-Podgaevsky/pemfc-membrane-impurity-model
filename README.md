...
Модель позволят выполнять расчет квазистационарного накопления примесей в системе рециркуляции водорода при закрытых клапанах удаления примесей.
...
- `n_o2_to_anode_mol_s > 0` означает, что $O_2$ идёт в анод;
- `n_n2_to_anode_mol_s > 0` означает, что $N_2$ идёт в анод;
- `n_h2_to_cathode_mol_s > 0` означает, что $H_2$ идёт в катод;
- `n_h2o_net_to_anode_mol_s > 0` означает, что суммарно вода идёт в анод;
